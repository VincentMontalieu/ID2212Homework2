...
public class BankImpl extends UnicastRemoteObject implements Bank {
	...
	public synchronized Account newAccount(String name) throws RemoteException,
		...
		if (account != null) {
			...
			throw new RejectedException("Rejected: se.kth.id2212.ex2.Bank: "
					+ bankName + " Account for: " + name + " already exists: "
					+ account);
		}
		account = new AccountImpl(name);
		accounts.put(name, account);
		System.out.println("se.kth.id2212.ex2.Bank: " + bankName + " Account: "
				+ account + " has been created for " + name);
		return account;
	}
